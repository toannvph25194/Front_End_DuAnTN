app.service("themsanPhamchitietService", function ($http) {
  ...
  this.getSanPhamTheoIds = function (IdSP) {
    return $http
      .get(
        "http://localhost:8080/api/admin-sanpham/hienthisanphamtheoid",
        { params: { IdSP: IdSP }, headers: config.headers }
      )
      .then(function (response) {
        return response.data;
    ...
    return $http
      .get(
        "http://localhost:8080/api/admin-image/hienthitatcaimage",
        { params: { IdSP: IdSP }, headers: config.headers }
      )
      .then(function (response) {
        return response.data;
  ...
  this.deleteImageById = function(imageId) {
    return $http.delete("http://localhost:8080/api/admin-image/delete-image/" + imageId, config)
      .then(function(response) {
        return response.data;
      })
...
app.controller(
  ...
  function ($scope, themsanPhamchitietService, $window, $route) {
    ...
    $scope.addSanPhamCT = function() {
      ...
      if ($scope.selectedsoluongton <= 0) {
        $scope.errorMessage.soluongton = "Số lượng phải là số dương và lớn hơn 0";
      }
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          themsanPhamchitietService.addSanPhamCT(sanPhamCTData).then(function(data) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          if ($scope.selectedImages.length > 0) {
            ...
            themsanPhamchitietService.getImageIds(maspInput).then(function(data) {
              ...
              if (totalImageCount > maxImageCount) {
                ...
                Swal.fire({
                  title: "Thông báo",
                  text: "Số lượng ảnh hiện có của sản phẩm là " + currentImageCount + ". Bạn chỉ được phép thêm tối đa " + (maxImageCount - currentImageCount) + " ảnh nữa.",
                  icon: "info",
                  position: "top-end",
                  toast: true,
                  showConfirmButton: false,
              ...
              themsanPhamchitietService.getThemAnh(imagesData).then(function(data) {
    ...
    $scope.updateCTSPSoluong = function(idsp, soluongton) {
      ...
      themsanPhamchitietService.updateCTSPSoluong(idsp, soluongton)
        .then(function(response) {
  ...
  $scope.deleteImage = function(imageId) {
    themsanPhamchitietService.deleteImageById(imageId)
      .then(function(data) {
      })
      .catch(function(data) {
